...
def is_valid(x, y):
    cx, cy = N // 2, N // 2  # center of grid
    scale = 2/ N  # scale to fit heart into grid

    # Convert grid coordinates to [-1, 1] range
    x_scaled = (x - cx) * scale
    y_scaled = (cy - y) * scale  # flip y so heart points down

    # Heart equation
    value = (x_scaled**2 + y_scaled**2 - 1)**3 - x_scaled**2 * y_scaled**3

    return (0 <= x < N and 0 <= y < N and value <= 0 and maze[x][y] == 0)
